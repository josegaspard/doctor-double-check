...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const action = url.searchParams.get('action') || 'page'; // 'page' or 'confirm'
    ...
    if (action === 'confirm') {
      ...
      return new Response(
        generateSuccessPage(doctorName, decoded.type),
...
function generateSuccessPage(doctorName: string, type: string): string {
  const typeLabels: Record<string, string> = {
    'all': 'todas las notificaciones',
    'content': 'notificaciones de nuevo contenido',
    'live': 'notificaciones de lives',
...
<body>
  <div class="card">
    ...
    <div class="content">
      ...
      <p style="font-size: 14px;">Puedes volver a activar las notificaciones desde la configuración de tu cuenta en cualquier momento.</p>
      <a href="https://doc-seek-relay.lovable.app/settings" class="btn">Ir a Configuración</a>
    </div>
    <div class="footer">
      © 2026 Dr Double Check. Todos los derechos reservados.
    </div>
