-- Update the process_consultation_purchase function to also create consultation and chat session records
...
BEGIN
  ...
  -- Check if an active chat session already exists
  ...
  IF v_chat_session_id IS NULL THEN
    ...
    VALUES (
      v_user_id, 
      COALESCE(v_user_role, 'patient')::chat_participant_type, 
      p_doctor_id, 
      'doctor', 
      'active', 
      false
