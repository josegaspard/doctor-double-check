...
export function InvoicePreviewModal({ isOpen, onClose, invoice }: InvoicePreviewModalProps) {
  ...
  useEffect(() => {
    ...
    const getSignedUrl = async () => {
      ...
      try {
        ...
        if (!data?.signedUrl) throw new Error(t('invoicePreviewModal.errorNoSignedUrl'));
        setSignedUrl(data.signedUrl);
      } catch (err: any) {
        console.error('Error getting signed URL:', err);
        setError(t('invoicePreviewModal.errorLoadingFile'));
        setSignedUrl(null);
      } finally {
        setIsLoading(false);
      }
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'approved':
      case 'paid':
        return <Badge variant="verified" className="gap-1"><CheckCircle className="w-3 h-3" />{t('invoicePreviewModal.statusApproved')}</Badge>;
      case 'pending':
      case 'processing':
        return <Badge variant="warning" className="gap-1"><Clock className="w-3 h-3" />{t('invoicePreviewModal.statusPending')}</Badge>;
      case 'rejected':
      case 'failed':
        return <Badge variant="destructive" className="gap-1"><XCircle className="w-3 h-3" />{t('invoicePreviewModal.statusRejected')}</Badge>;
      default:
      ...
      <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <DialogTitle>{t('invoicePreviewModal.title')} {invoice.invoice_number}</DialogTitle>
            </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-muted/50 rounded-lg">
            <div>
              <p className="text-xs text-muted-foreground">{t('invoicePreviewModal.period')}</p>
            ...
            <div>
              <p className="text-xs text-muted-foreground">{t('invoicePreviewModal.amount')}</p>
              <p className="text-sm font-medium flex items-center gap-1">
            ...
            <div>
              <p className="text-xs text-muted-foreground">{t('invoicePreviewModal.file')}</p>
              <p className="text-sm font-medium truncate">{invoice.file_name}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">{t('invoicePreviewModal.uploaded')}</p>
          ...
          {invoice.status === 'rejected' && invoice.admin_notes && (
            <div className="p-4 bg-destructive/10 border border-destructive/20 rounded-lg">
              <p className="text-sm font-medium text-destructive mb-1">{t('invoicePreviewModal.rejectionReason')}</p>
          ...
          <div className="border rounded-lg overflow-hidden">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center h-48 bg-muted">
                ...
                <p className="text-sm text-muted-foreground">{error || t('invoicePreviewModal.couldNotLoadFile')}</p>
              </div>
            ) : isPdf ? (
              <iframe
          ...
          <div className="flex flex-col sm:flex-row gap-2 pt-2">
            ...
            >
              ...
              {t('invoicePreviewModal.openInNewTab')}
            </Button>
            <Button 
              variant="outline"
              disabled={!signedUrl}
            ...
            >
              ...
              {t('invoicePreviewModal.download')}
            </Button>
          </div>
        </div>
      </DialogContent>
