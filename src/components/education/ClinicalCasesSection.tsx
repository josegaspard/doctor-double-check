// Casos clínicos de Aprendizaje (pestaña "casos"), 11-sep-2026.
//
// Extraído de MedicalEducation.tsx sin cambiar su comportamiento: sigue siendo
// el feed de `clinical_cases` para discusión entre doctores y residentes. Lo
// único nuevo es la revisión antes de publicar (regla del encargo: ninguna
// acción con consecuencias se dispara al primer clic).
...
import { Plus, MessageCircle, Eye, Stethoscope, Send, Loader2, Trash2 } from 'lucide-react';
...
export default function ClinicalCasesSection() {
  ...
  const filtered = useMemo(() => {
    if (!specialty || specialty === 'Todas') {
      // Recomendación: sin filtro, priorizar casos afines a los intereses.
  ...
  const submitCase = async () => {
    ...
    // Publicar es una acción con consecuencias (queda visible para el resto):
    // revisión antes de ejecutar, el primer clic nunca guarda.
    const ok = await confirm({
      title: t('mm2.learning.cases.confirmTitle'),
      description: t('mm2.learning.cases.confirmDesc'),
      details: [
        { label: t('medicalEducationPage.fieldTitle'), value: form.title.trim() },
        { label: t('medicalEducationPage.fieldSpecialty'), value: form.specialty },
      ],
      confirmLabel: t('medicalEducationPage.publishCase'),
    });
    if (!ok) return;
  ...
  // Doctores, residentes y admin pueden subir casos.
