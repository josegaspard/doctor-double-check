...
export default function AdminAds() {
  ...
  const [newPlacement, setNewPlacement] = useState({
    name: '', display_name: '', description: '', width: 728, height: 90, format: 'banner',
  });
  ...
  useEffect(() => {
    setConfigForm({
      ...
      cpm_rate: config.cpm_rate,
      cpc_rate: config.cpc_rate,
  ...
  const fetchCampaigns = useCallback(async () => {
    ...
    const { data: payments } = await supabase
      .from('ad_payments' as any)
      .select('amount')
      .eq('status', 'paid');
    setTotalRevenue((payments as any[] || []).reduce((sum: number, p: any) => sum + Number(p.amount), 0));
  ...
  const saveConfig = async () => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        cpm_rate: configForm.cpm_rate,
        cpc_rate: configForm.cpc_rate,
        min_budget: configForm.min_budget,
        max_file_size_kb: configForm.max_file_size_kb,
  ...
  const addPlacement = async () => {
    ...
    await supabase.from('ad_placements' as any).insert({
      ...newPlacement,
      ...
      <div className="container mx-auto px-3 sm:px-4 py-4 sm:py-6 max-w-6xl">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              {es ? 'Administra campañas, placements y configuración' : 'Manage campaigns, placements and settings'}
            </p>
          </div>
        </div>
        ...
        <Tabs defaultValue="dashboard">
          <TabsList className="w-full sm:w-auto grid grid-cols-4 sm:flex mb-4">
            ...
            <TabsTrigger value="placements" className="gap-1.5 text-xs sm:text-sm">
              ...
              <Card>
                <CardContent className="p-4">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    {totalClicks} clics / {totalImpressions} impresiones
                  </p>
                </CardContent>
              </Card>
            )}
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                              ...
                              <span className="flex items-center gap-1">
                                ...
                                {stats.impressions.toLocaleString()} imp
                              </span>
                              <span className="flex items-center gap-1">
                                <MousePointerClick className="w-3 h-3" />
                                {stats.clicks} clics ({ctr}%)
                              </span>
                              <span className="flex items-center gap-1">
                              ...
                              {campaign.start_date && (
                                <span className="flex items-center gap-1">
                                  ...
                                  {campaign.end_date && ` - ${format(new Date(campaign.end_date), 'dd/MM/yy')}`}
                                </span>
                              )}
                            </div>
                          </div>
          ...
          <TabsContent value="placements">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-base">Nuevo Placement</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Label className="text-xs">Nombre (slug)</Label>
                    <Input value={newPlacement.name} onChange={e => setNewPlacement(p => ({ ...p, name: e.target.value }))} placeholder="hero_banner" />
                  </div>
                  <div>
                    <Label className="text-xs">Nombre visible</Label>
                    <Input value={newPlacement.display_name} onChange={e => setNewPlacement(p => ({ ...p, display_name: e.target.value }))} placeholder="Hero Banner" />
                  </div>
                  <div>
                    <Label className="text-xs">Ancho (px)</Label>
                    <Input type="number" value={newPlacement.width} onChange={e => setNewPlacement(p => ({ ...p, width: parseInt(e.target.value) || 728 }))} />
                  </div>
                  <div>
                    <Label className="text-xs">Alto (px)</Label>
                    <Input type="number" value={newPlacement.height} onChange={e => setNewPlacement(p => ({ ...p, height: parseInt(e.target.value) || 90 }))} />
                  </div>
                </div>
                <Input value={newPlacement.description} onChange={e => setNewPlacement(p => ({ ...p, description: e.target.value }))} placeholder="Descripción..." />
                <Button onClick={addPlacement} disabled={!newPlacement.name || !newPlacement.display_name}>
                  Crear Placement
                </Button>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-5">
                <div className="flex items-center justify-between">
                  <div>
                    <Label className="font-semibold">Sistema de Publicidad</Label>
                    <p className="text-xs text-muted-foreground">Activa o desactiva toda la publicidad</p>
                  </div>
                  <Switch
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label className="text-xs">CPM (por 1,000 impresiones)</Label>
                    <Input type="number" value={configForm.cpm_rate} onChange={e => setConfigForm(f => ({ ...f, cpm_rate: Number(e.target.value) }))} />
                  </div>
                  <div>
                    <Label className="text-xs">CPC (por clic)</Label>
                    <Input type="number" value={configForm.cpc_rate} onChange={e => setConfigForm(f => ({ ...f, cpc_rate: Number(e.target.value) }))} />
                  </div>
                  <div>
                    <Label className="text-xs">Presupuesto Mínimo</Label>
                    <Input type="number" value={configForm.min_budget} onChange={e => setConfigForm(f => ({ ...f, min_budget: Number(e.target.value) }))} />
                  </div>
                  <div>
                    <Label className="text-xs">Tamaño Máx. Archivo (KB)</Label>
                    <Input type="number" value={configForm.max_file_size_kb} onChange={e => setConfigForm(f => ({ ...f, max_file_size_kb: Number(e.target.value) }))} />
                  </div>
                </div>
