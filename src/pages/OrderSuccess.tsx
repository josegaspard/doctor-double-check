          ...
          <Card className="mb-6 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-200">
            <CardContent className="p-5">
              <div className="flex items-center gap-2 mb-4 text-xs text-muted-foreground">
                ...
                <span>{t('autoI18n.orderSuccess3')}</span>
              </div>
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    {t('autoI18n.orderSuccess4')}: {order.quantity}
                  </p>
                  ...
                  {order.delivery_fee > 0 && (
                    <p className="text-xs text-muted-foreground mt-1">
                      + ${Number(order.delivery_fee).toLocaleString()} {t('autoI18n.orderSuccess5')}
                    </p>
                  )}
                </div>
              </div>
              <div className="mt-4 pt-4 border-t">
                <Badge className="bg-primary text-primary-foreground border-primary border text-xs">
                  ...
                  {t('autoI18n.orderSuccess6')}
                </Badge>
                {order.shipping_city && (
                  <p className="text-xs text-muted-foreground mt-2">
                    📍 {t('autoI18n.orderSuccess7')}: {order.shipping_city}{order.shipping_state ? `, ${order.shipping_state}` : ''}
                  </p>
                )}
              </div>
            </CardContent>
