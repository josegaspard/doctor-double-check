...
export default function AdminResidents() {
  ...
  const fetchResidents = async () => {
    try {
      ...
      toast({ title: 'Error', description: 'No se pudieron cargar los residentes', variant: 'destructive' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      toast({
        title: actionDialog.action === 'approve' ? 'Residente aprobado' : 'Residente rechazado',
        description: `${actionDialog.resident.profile?.name} ha sido ${newStatus === 'approved' ? 'aprobado' : 'rechazado'}`,
      });
      ...
      <div className="container mx-auto px-4 py-6 max-w-6xl">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <h3 className="font-semibold">{resident.profile?.name || 'Sin nombre'}</h3>
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          ...
                          <Badge variant="outline">Año {resident.year}</Badge>
                        </div>
                        <p className="text-sm text-muted-foreground">
                          Institución: {resident.institution}
                        </p>
                        {resident.cedula_profesional && (
                          <p className="text-xs text-muted-foreground">
                            Cédula: {resident.cedula_profesional}
                          </p>
                        )}
                        <p className="text-xs text-muted-foreground">
                          Registrado: {new Date(resident.created_at).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
        ...
        <AlertDialog open={actionDialog.open} onOpenChange={(open) => setActionDialog({ ...actionDialog, open })}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>
                {actionDialog.action === 'approve' ? '¿Aprobar residente?' : '¿Rechazar residente?'}
              </AlertDialogTitle>
              <AlertDialogDescription>
                {actionDialog.action === 'approve'
                  ? `${actionDialog.resident?.profile?.name} podrá acceder a funcionalidades de residente verificado.`
                  : `${actionDialog.resident?.profile?.name} no será verificado como residente.`}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
