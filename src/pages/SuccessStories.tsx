    ...
    <div className="font-sans text-slate-800 bg-slate-50 min-h-screen selection:bg-[#00768b] selection:text-white">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-12">
          <div className="text-center max-w-3xl mx-auto">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 border border-white/20 backdrop-blur-sm mb-6">
              <Award className="w-4 h-4 text-[#aed3d9]" />
              <span className="text-xs font-bold uppercase tracking-widest text-[#aed3d9]">Casos de Éxito</span>
            </div>
        ...
        <div className="container mx-auto px-4 sm:px-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8">
            <div className="text-center">
              <p className="text-2xl sm:text-3xl font-bold text-[#163a83]">5,000+</p>
              <p className="text-xs sm:text-sm text-gray-500 mt-1">Médicos activos</p>
            </div>
            <div className="text-center">
              <p className="text-2xl sm:text-3xl font-bold text-[#00768b]">15M+</p>
              <p className="text-xs sm:text-sm text-gray-500 mt-1">Pacientes atendidos</p>
            </div>
            <div className="text-center">
              <p className="text-2xl sm:text-3xl font-bold text-[#163a83]">98%</p>
              <p className="text-xs sm:text-sm text-gray-500 mt-1">Satisfacción</p>
            </div>
            <div className="text-center">
              <p className="text-2xl sm:text-3xl font-bold text-[#00768b]">24/7</p>
              <p className="text-xs sm:text-sm text-gray-500 mt-1">Disponibilidad</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 sm:px-6">
          <div className="grid gap-8 sm:gap-12">
            {stories.map((story, index) => (
              <div 
                key={index}
                className={`flex flex-col ${index % 2 === 1 ? 'md:flex-row-reverse' : 'md:flex-row'} gap-6 sm:gap-12 items-center bg-white rounded-2xl p-6 sm:p-10 shadow-lg border border-gray-100`}
              >
                <div className="w-full md:w-1/3 flex flex-col items-center">
                  <img 
                    src={story.image} 
                    alt={story.name}
                    className="w-24 h-24 sm:w-32 sm:h-32 rounded-full object-cover border-4 border-[#aed3d9] shadow-lg"
                  />
                  <h3 className="mt-4 text-lg sm:text-xl font-bold text-gray-800 text-center">{story.name}</h3>
                  <p className="text-sm text-[#00768b] font-medium">{story.specialty}</p>
                  
                  <div className="flex items-center gap-1 mt-2">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} className="w-4 h-4 text-yellow-400 fill-yellow-400" />
                    ))}
                    <span className="text-sm font-bold text-gray-600 ml-1">{story.stats.rating}</span>
                  </div>
                </div>
                
                <div className="w-full md:w-2/3">
                  <Quote className="w-8 h-8 sm:w-10 sm:h-10 text-[#aed3d9] mb-4" />
                  <p className="text-base sm:text-xl text-gray-700 italic leading-relaxed mb-6">
                    "{story.quote}"
                  </p>
                  
                  <div className="grid grid-cols-3 gap-4">
                    <div className="bg-gray-50 rounded-lg p-3 sm:p-4 text-center">
                      <Users className="w-5 h-5 text-[#163a83] mx-auto mb-1" />
                      <p className="text-lg sm:text-xl font-bold text-gray-800">{story.stats.patients}</p>
                      <p className="text-xs text-gray-500">Pacientes</p>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-3 sm:p-4 text-center">
                      <TrendingUp className="w-5 h-5 text-[#00768b] mx-auto mb-1" />
                      <p className="text-lg sm:text-xl font-bold text-gray-800">{story.stats.consultations}</p>
                      <p className="text-xs text-gray-500">Consultas</p>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-3 sm:p-4 text-center">
                      <Star className="w-5 h-5 text-yellow-400 mx-auto mb-1" />
                      <p className="text-lg sm:text-xl font-bold text-gray-800">{story.stats.rating}</p>
      ...
      <section className="py-16 sm:py-24 bg-gradient-to-br from-[#163a83] to-[#00768b]">
        <div className="container mx-auto px-4 sm:px-6 text-center">
          <h2 className="text-2xl sm:text-3xl font-bold text-white mb-4">¿Listo para ser parte del éxito?</h2>
